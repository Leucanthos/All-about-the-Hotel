...
DEFAULT_REVERSE_QUERIES = DATA_ROOT / "reverse_queries.csv"
DEFAULT_COMMENTS = DATA_ROOT / "filtered_comments.csv"
DEFAULT_SUMMARIES = DATA_ROOT / "category_summaries.json"
DEFAULT_OUT_DIR = DATA_ROOT / "sft"
...
def load_comments(path: Path):
    ...
    for row in read_csv(path):
        cid = row.get("_id") or row.get("comment_id")
        if not cid:
            continue
        row["comment"] = short_text(row.get("comment", ""))
        comments[cid] = row
