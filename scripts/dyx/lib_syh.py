...
class HotelReviewRAG:
    ...
    def __init__(self, api_key: str, dashvector_api_key: str, dashvector_endpoint: str, 
        ...
        self.intent_categories = {
            '房间': ['床', '房间', '卧室', '设施', '装修', '隔音', '空间', '安静', '噪音', '卫生'],
            '服务': ['服务', '前台', '前台员工', '态度', '效率', '热情', '入住', '退房'],
            '早餐': ['早餐', '自助', '点心', '虾饺', '肠粉', '品种', '餐饮'],
            '设施': ['泳池', '健身房', 'gym', '游泳', '温度', '博物馆', '花园'],
            '位置': ['位置', '地理', '地段', '地铁', '交通', '方便'],
        }

        # 加载类别摘要JSON，用于返回原始纯净摘要文本（与主分支 category_summaries.json 对齐）
        summaries_json_path = self.data_dir / "category_summaries.json"
        if summaries_json_path.exists():
            import json as _json
            with open(summaries_json_path, 'r', encoding='utf-8') as _f:
                _rows = _json.load(_f)
            self._summaries_json = {r.get('category', ''): r for r in _rows if r.get('category')}
        else:
            self._summaries_json = {}
...
请只输出评论内容，用换行符分隔。"""
    ...
    def retrieve_summary_by_intent(self, query: str, intent: Dict) -> List[Dict]:
        ...
        优化A：相似度阈值过滤（0.25），避免低相关类别污染 LLM prompt
        优化B：意图规则补充，将关键词命中的类别强制加入结果
        QA变体：若知识库包含 qa_variant 文档，自动参与向量匹配并按类别去重
        """
        # 优化B：意图→摘要类别映射（与主分支 CATEGORY_ALIASES 对齐，扩展为 14 个词条）
        INTENT_CATEGORY_MAP = {
            '早餐': ['餐饮设施'],
            '餐饮': ['餐饮设施'],
            '房间': ['房间设施', '卫生状况', '安静程度', '景观/朝向'],
            '服务': ['前台服务', '客房服务', '退房/入住效率', '整体满意度'],
            '设施': ['公共设施'],
            '位置': ['交通便利性', '周边配套'],
            # 以下为主分支 CATEGORY_ALIASES 新增别名
            '前台': ['前台服务', '退房/入住效率'],
            '入住': ['退房/入住效率', '前台服务'],
            '卫生': ['卫生状况', '房间设施'],
            '噪音': ['安静程度', '房间设施'],
        ...
        SCORE_THRESHOLD = 0.25  # 优化A：低于此分数的结果过滤掉

        try:
            # 用 intent_categories 关键词确定主导意图类别
            query_lower = query.lower()
            kw_scores = {
                cat: sum(1 for kw in kws if kw in query_lower)
                for cat, kws in self.intent_categories.items()
            }
            dominant_intent = max(kw_scores, key=kw_scores.get) if max(kw_scores.values()) > 0 else None
            rule_categories = INTENT_CATEGORY_MAP.get(dominant_intent, [])

            # 向量检索：候选池扩大到8（覆盖可能的 QA 变体文档）
            query_embedding = self.embedding_client.embed_query(query)
            ...
            # 优化A：阈值过滤 + 按 category 去重（同类别保留最高分）
            retrieved_by_cat = {}
            for i, (doc_id, document, metadata) in enumerate(zip(ids, documents, metadatas)):
                ...
                if score < SCORE_THRESHOLD:
                    continue
                if category not in retrieved_by_cat or retrieved_by_cat[category]['score'] < score:
                    # 优先从 JSON 取纯净摘要文本；ChromaDB document 是嵌入增强文本，不适合直接展示
                    clean_summary = self._summaries_json.get(category, {}).get('summary', document)
                    retrieved_by_cat[category] = {
                        'rank': i + 1,
                        'method': 'summary_vector',
                        'doc_id': doc_id,
                        'score': score,
                        'category': category,
            ...
            # 优化B：意图规则——若命中类别不在向量结果中，强制补充
            if rule_categories:
                try:
                    rule_docs = self.summary_collection.get(
                        where={"category": {"$in": rule_categories}}
                    )
                    seen_rule_cats = set()
                    for j, metadata in enumerate(rule_docs.get('metadatas', [])):
                        rule_cat = metadata.get('category', '')
                        if rule_cat not in retrieved_by_cat and rule_cat not in seen_rule_cats:
                            seen_rule_cats.add(rule_cat)
                            # 规则补充也从 JSON 获取纯净摘要
                            clean_rule_summary = self._summaries_json.get(rule_cat, {}).get('summary', rule_docs['documents'][j])
                            retrieved_by_cat[rule_cat] = {
                                'rank': 99,
                                'method': 'summary_rule',
                                'doc_id': rule_docs['ids'][j],
                                'score': 0.0,
                                'category': rule_cat,
                                'keywords': metadata.get('keywords', ''),
                                'comment_count': metadata.get('comment_count', 0),
                                'summary': clean_rule_summary
                            }
                except Exception:
                    pass  # 规则查询失败不影响向量结果

            # 排序：向量结果按分数降序，规则结果附后；最多返回3条
            vector_part = sorted(
                [v for v in retrieved_by_cat.values() if v['method'] == 'summary_vector'],
                key=lambda x: x['score'], reverse=True
            )
            rule_part = [v for v in retrieved_by_cat.values() if v['method'] == 'summary_rule']
            sorted_results = (vector_part + rule_part)[:3]
...
回答要有条理，使用恰当的标题分隔内容。"""
    ...
    def query(self, query: str, enable_hyde: bool = True, topk: int = 3) -> Dict:
        ...
        # 兼容 exp3-syh.ipynb 接口：contexts 为纯文本列表
        result['contexts'] = [item.get('comment', '') for item in reranked]
...
def print_rag_result(result: Dict, verbose: bool = False):
    ...
    if result.get('retrieval_summaries'):
        ...
        for i, summary in enumerate(result['retrieval_summaries']):
            ...
            method_tag   = '[规则]' if method == 'summary_rule' else f'[向量 {score:.4f}]'
            print(f"  [{i+1}] {category}  {method_tag}  (评论数: {comment_count})")
            if summary_text:
