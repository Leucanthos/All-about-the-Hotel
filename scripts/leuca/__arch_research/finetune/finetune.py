...
# ⚠️ ARCHIVED — 微调实验框架，不进入生产调用链。运行需在项目根目录 (CWD = Project/)
    ...
    def __init__(self, processor, df, store, cfg: ExperimentConfig, max_pairs=800):
        ...
        for img_id, meta in zip(data["ids"], data["metadatas"]):
            ...
            if row_idx < len(df):
                ...
                img_path = os.path.join(_PROJ, "data", "images", f"{img_id}.jpg")
...
def train(cfg: ExperimentConfig):
    ...
    store = NumpyVectorStore(os.path.join(_PROJ, "data", "vectors_np"))
    df = pd.read_parquet(os.path.join(_PROJ, "data", "hotel_reviews_table.parquet"))
    ...
    save_dir = os.path.join(_PROJ, "data", "experiments", cfg.name)
    ...
    if cfg.name == "baseline":
        default_dir = os.path.join(_PROJ, "data", "finetuned")
...
def rebuild_index(exp_name: str = "baseline"):
    ...
    exp_dir = os.path.join(_PROJ, "data", "experiments", exp_name)
    ft_path = os.path.join(exp_dir, "projection_heads.pt")
    if not os.path.exists(ft_path):
        ft_path = os.path.join(_PROJ, "data", "finetuned", "projection_heads.pt")
    ...
    df = pd.read_parquet(os.path.join(_PROJ, "data", "hotel_reviews_table.parquet"))
    store = NumpyVectorStore(os.path.join(_PROJ, "data", "vectors_np"))
    img_dir = os.path.join(_PROJ, "data", "images")
