...
from _shared.store import NumpyVectorStore
from _shared.data import PROJECT_ROOT as _PROJ, VECTORS_DIR, COMMENTS_PATH
...
class MultimodalRetriever:
    """Chinese-CLIP 多模态检索引擎 — NumPy 后端，线程安全."""
    ...
    def _load_model(self):
        ...
        ft_dir = os.path.join(str(_PROJ), "data", "finetuned")
        ...
        self._ov_text = self._ov_vision = None
        self._ov_text_names = self._ov_vision_names = None
    ...
    def comments(self) -> pd.DataFrame:
        if self._comment_df is None:
            self._comment_df = pd.read_parquet(str(COMMENTS_PATH))
        return self._comment_df

    def _try_load_openvino(self):
        ft_weights = os.path.join(str(_PROJ), "data", "finetuned", "projection_heads.pt")
        if os.path.exists(ft_weights):
            print("  [Fine-tuned weights detected, using PyTorch (skip OpenVINO)]")
            return
        try:
            import openvino as ov
            ov_dir = os.path.join(str(_PROJ), "data", "openvino")
    ...
    def _dump(self, results, top_k: int) -> list[dict]:
        ...
        for i in range(min(top_k, len(results["ids"][0]))):
            ...
            out.append({
                "rank": i + 1, "similarity": round(sim, 4),
                "comment": meta.get("comment") or meta.get("content", "")[:200],
                "score": meta.get("score"), "room_type": meta.get("room_type"),
    ...
    def search_images_batch(self, queries: list[str], top_k: int = 5) -> list[list[dict]]:
        ...
        return [self._dump(self.store.query("hotel_images",
                query_embeddings=[e.tolist()], n_results=top_k), top_k) for e in embs]
    ...
    def search_by_image_batch(self, images: list[Union[str, Image.Image]],
        ...
        return [self._dump(self.store.query("hotel_comments",
                query_embeddings=[e.tolist()], n_results=top_k), top_k) for e in embs]
    ...
    def stats(self) -> dict:
        try:
            return {
                ...
                "comments_total": len(self.comments) if self._comment_df is not None else 0,
            }
        except Exception:
