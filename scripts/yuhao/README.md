...
scripts/yuhao/
├── README.md
├── agentic/                  ← Agentic RAG 检索与问答
│   ├── __init__.py
│   ├── rag.py                ← HotelCorpus 检索引擎 + AgenticRAG 流水线
│   └── interfaces.py         ← Layer1/Layer2 接口适配
├── sft/                      ← SFT 数据构建与微调训练
│   ├── __init__.py
│   ├── build.py              ← SFT 数据集构建脚本
│   ├── train.py              ← QLoRA/Unsloth 训练脚本
│   └── prompts.md            ← 提示词设计文档
│
data/                         ← 仓库级共享数据
  hotel_reviews_table.parquet     ← 评论数据 (RAG 直接读取)
  category_summaries.json         ← 类别摘要
