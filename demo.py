...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:16px">
  <div style="background:var(--card);border-radius:10px;padding:14px">
    <h3 style="font-size:14px;color:var(--brown);margin-bottom:10px">图片 → 文本</h3>
  ...
  <div style="background:var(--card);border-radius:10px;padding:14px">
    <h3 style="font-size:14px;color:var(--brown);margin-bottom:10px">房型分类</h3>
    <div style="display:flex;gap:8px;margin-bottom:8px">
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if p.path in ("/", "/index.html"):
            ...
            try:
                r = ({"rerank": _api.prompt2image_rerank,
                      "filter": _api.prompt2image_filtered,
                      "granular": lambda q, t: _api.prompt2image_granular(q, topK=t, granularity="room_type")}
                     .get(m, _api.prompt2image))(q, topK=top))
                self._json(r)
            ...
            try:
                self._json(_api.classify_room_type(qs.get("text", [""])[0]))
            except Exception as e:
                self._json({"error": str(e)}, 500)

        elif p.path == "/api/stats":
            self._json(_api.stats)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="宿说 Web Demo")
